...
namespace DnsServerCore.Dns
{
    public class DnsServer : IDisposable
    {
        ...
        NetProxy[] _proxy;
        NetProxy[] _reliableProxy;
        NameServerAddress[] _forwarders;
        NameServerAddress[] _reliableForwarders;
        DnsTransportProtocol _forwarderProtocol = DnsTransportProtocol.Udp;
        DnsTransportProtocol _reliableForwarderProtocol = DnsTransportProtocol.Udp;
                ...
                ThreadPool.QueueUserWorkItem(delegate (object state)
                {
                    ...
                    try
                    {
                        if ((viaNameServers == null) && (_forwarders != null))
                        {
                            ...
                            if (_reliableProxy == null)
                            {
                                if (_reliableForwarders != null)
                                {
                                    foreach (NameServerAddress nameServerAddress in _reliableForwarders)
                                    {
                                        if (nameServerAddress.IsIPEndPointStale) //refresh forwarder IPEndPoint if stale
                                            nameServerAddress.RecursiveResolveIPAddress(_dnsCache, null, _preferIPv6, _retries, _timeout);
                                    }
                            ...
                            if (ChinaDnsServer.GfwList.Instance.IsInList(request.Question[0].Name))
                            {
                                response = ResolveQuestionReliable(request.Question[0]);
                            }
                            else
                            {
                                response = ResolveQuestion(request.Question[0]);

                                if (response.Answer != null && response.Answer.Length > 0 && _reliableForwarders != null)
                                {
                                    bool isTypeA = false;
                                    bool isInIPList = false;
                                    foreach (DnsResourceRecord answer in response.Answer)
                                    {
                                        if (answer.Type == DnsResourceRecordType.A)
                                        {
                                            isTypeA = true;
                                            if (ChinaDnsServer.IPList.Instance.IsInList(answer.RDATA.ToString()))
                                            {
                                                isInIPList = true;
                                                break;
                                            }
        ...
        private DnsDatagram ResolveQuestion(DnsQuestionRecord dnsQuestionRecord)
        {
            ...
            return dnsClient.Resolve(dnsQuestionRecord);
        }

        private DnsDatagram ResolveQuestionReliable(DnsQuestionRecord dnsQuestionRecord)
        {
            DnsClient dnsClient = new DnsClient(_reliableForwarders);

            dnsClient.Proxy = _reliableProxy;
            dnsClient.PreferIPv6 = _preferIPv6;
            dnsClient.Protocol = _reliableForwarderProtocol;
            dnsClient.Retries = _retries;
            dnsClient.Timeout = _timeout;
        ...
        public NetProxy[] ReliableProxy
        {
            get { return _reliableProxy; }
            set { _reliableProxy = value; }
        }
        ...
        public NameServerAddress[] ReliableForwarders
        {
            get { return _reliableForwarders; }
            set { _reliableForwarders = value; }
        }
        ...
        public DnsTransportProtocol ReliableForwarderProtocol
        {
            get { return _reliableForwarderProtocol; }
            set { _reliableForwarderProtocol = value; }
        }
