...
namespace TechnitiumLibrary.Net.Dns
{
    ...
    public class DnsClient
    {
        ...
        public DnsDatagram Resolve(DnsDatagram request)
        {
            ...
            if (_proxy != null && _proxy.Length > 1)
            {
                retries *= _proxy.Length; //retries on per proxy basis

                byte[] select = new byte[1];
                _rnd.GetBytes(select);

                nextProxyIndex = select[0] % _proxy.Length;
            }
            ...
            while (retry < retries)
            {
                ...
                {
                    proxy = _proxy[nextProxyIndex];
                    nextProxyIndex = (nextProxyIndex + 1) % _proxy.Length;
                }

                if (server.IsIPEndPointStale && (_proxy == null || _proxy.Length == 0))
                {
                ...
                try
                {
                    ...
                    if ((proxy != null) && (protocol == DnsTransportProtocol.Udp) && !proxy.IsUdpAvailable() && !proxy.IsProxyBypassed(server.EndPoint))
                        protocol = DnsTransportProtocol.Tcp;

                    DnsClientConnection connection = DnsClientConnection.GetConnection(protocol, server, proxy);
